 ...
 */
export function* enumarteCombination<T>(items: readonly T[], count: number, isRepitision: boolean = false): IterableIterator<readonly T[]>
{
    if (count < 0 || count > items.length) throw Error("'count' should be a non-negative integer and should not be more than 'items.length'.");
    
    function* enumCombin<T>(items: readonly T[], count: number, isRepitision: boolean = false): IterableIterator<readonly T[]>
    {
        if (count === 0)
        {
            yield [];
        ...
        else
        {
            ...
            {
                for (const restItems of enumCombin(items.slice(isRepitision ? index : index + 1, items.length), count - 1))
                {
